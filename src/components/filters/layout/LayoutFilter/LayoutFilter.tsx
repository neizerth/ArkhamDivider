...
export const LayoutFilter = () => {
  ...
  const haveColor = uniq(colorLayouts.map(prop('color')).filter(isNotNil)).length > 1;

  const haveOrientation =
    uniq(orientationLayouts.map(prop('orientation')).filter(isNotNil)).length > 1;
  ...
  return (
    <div className={S.container}>
      ...
      {categoryId && (
        <>
          {haveOrientation && <LayoutOrientationToggle data={orientationLayouts} />}
          {haveColor && <LayoutColorToggle className={S.color} data={colorLayouts} />}
