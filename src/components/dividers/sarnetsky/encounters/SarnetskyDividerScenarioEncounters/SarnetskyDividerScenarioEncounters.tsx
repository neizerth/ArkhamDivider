...
export const SarnetskyDividerScenarioEncounters = (
  props: SarnetskyDividerScenarioEncountersBaseProps & {
    scenario: IScenario;
...
export const SarnetskyDividerGroupedEncounters = ({
  ...
  const getGroupClass = ({ length }: string[]) => {
    ...
    return [S.group, closedToMax ? S.group_size_max : S.group_size_normal];
  };

  const wrapperClassName = classNames(S.wrapper, between(totalSize) && S.wrapper_max);
    ...
    <div className={classNames(S.container, showName && S.withName)}>
      ...
      <div className={wrapperClassName}>
        ...
        {sideEncounters.length > 0 && (
          <div className={classNames(S.side, getGroupClass(sideEncounters))} style={groupStyle}>
            {sideEncounters.map((icon) => (
              <div className={classNames(S.encounter, S.encounter_side)} key={`side_${icon}`}>
                <Icon icon={icon} />
              </div>
            ))}
          </div>
