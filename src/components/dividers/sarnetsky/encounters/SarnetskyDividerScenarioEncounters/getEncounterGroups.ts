...
export const getEncounterGroups = (options: IGetEcnounterGroups) => {
  ...
  const groups = values(groupBy(prop('version_text'), ecnounterGroups)).filter(isNotNil);

  const versionGroups = groups.map((groups) => {
    const [first] = groups;
    const { version_number, version_text } = first;
    const main = uniq(
      groups.filter(propEq(true, 'is_default')).map(prop('encounter_sets')).filter(isNotNil).flat()
    );
...
export const getScenarioGroups = ({
  ...
  const { icon, encounter_sets = [], extra_encounter_sets = [] } = scenario;

  const toIcon = (code: string) => encounterSets.find(propEq(code, 'code'))?.icon;
...
const getAllGroups = ({ scenario, mainScenario, encounterSets }: IGetEcnounterGroups) => {
