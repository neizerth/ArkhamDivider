...
export const ArkhamDecoSideXP = ({ xpCost }: ArkhamDecoSideXPProps) => {
  const { level, max = level } = xpCost;

  return (
    <div className={S.container}>
      {range(1, MAX_XP + 1).map((l) => (
        <div className={classNames(S.level, l <= max && S.level_full)} key={l}>
          {l > max && <SmallBar className={classNames(S.bar, S.small, S.inactive)} />}
          {l <= max && <FullBar className={classNames(S.bar, S.full, l <= level && S.current)} />}
        </div>
      ))}
    </div>
  );
