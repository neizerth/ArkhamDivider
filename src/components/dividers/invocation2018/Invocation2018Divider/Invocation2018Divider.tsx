...
export const Invocation2018Divider = (props: Invocation2018DividerProps) => {
  const { faction, xpCost, displaySideXP, displayNumericXP = false, name = '' } = props;
  ...
  return (
    <ClassicDivider
      ...
      titleStroke={false}
      titleClassName={classNames(S.title, S[`title_${orientation}`], S[`title_${realLanguage}`])}
      cornerRadiusClassName={S.cornerRadius}
      background={background?.src}
    >
      <div className={classNames(S.icon, xpCost && S.icon_xp)} onClick={selectIcon}>
        {icon && (
          <>
