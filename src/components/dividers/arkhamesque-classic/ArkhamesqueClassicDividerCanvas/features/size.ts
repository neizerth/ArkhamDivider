...
import { IBox, IBoxOffset } from "@/types/units";
...
export const getCanvasSize = ({
  ...
  const entryCavasSizes = icons.map(
    item => getEntryCanvasSize(item, defaultCanvasSize)
  );

  // console.log(entryCavasSizes);

  const canvasSize = entryCavasSizes.reduce(
    (target, canvasSize) => {
      if (
        canvasSize.width > target.width || 
        canvasSize.height > target.height
      ) {
        return canvasSize;
      }
