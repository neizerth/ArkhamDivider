...
import { getArkhamesqueClassicStory } from '@/shared/store/features/dividers/arkhamesque/criteria';
...
export const getStoryDividerData = ({ data, divider }: GetDividerDataOptions) => {
  ...
  if (scenario && !isEncounter) {
    return {
      scenario,
      category,
      image: getDividerImage([data.prefix, categoryPrefix, name, scenario.name]),
    };
  }
  ...
  return {
    scenario,
    category,
    image: getDividerImage([data.prefix, categoryPrefix, name, campaign.name]),
  };
};
