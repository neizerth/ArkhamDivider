...
const XPNumber = ({ value }: { value: number }) => {
  return <span className={classNames(S.number, S[`number_${value}`])}>{value}</span>;
};
  ...
  return (
    <span
      className={classNames(
        S.container,
        xpCost.max ? [xpCost.max === MAX_XP ? S.withPlus : S.withRange] : S.single
      )}
    >
      ...
      {xpCost.max && (
        <>
          {xpCost.max === MAX_XP && (
            <span className={classNames(S.plus, S[`plus_${xpCost.level}`])}>+</span>
          )}
          {xpCost.max < MAX_XP && (
            <>
