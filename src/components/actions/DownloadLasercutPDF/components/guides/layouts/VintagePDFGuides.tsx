import { getTabPosition, TabPosition } from "@/components/dividers/vintage/VintageDivider/features/tabPosition"
...
export const VintagePDFGuides = ({
  ...
  const tabLeft = Math.round(
    toPrintSize(TAB_RECTS[tabPosition])
  );
  ...
  const topBodyLeftCorner = topCornerRadius > 0 && tabPosition !== 'left' ? [
  ...
  const tabStart = tab.x > 0 ? [
    ['L', x + tab.x, y + tab.height]
  ] : [];

  const tabLeftCorner = radius > 0 ? [
    ['L', x + tab.x, y + radius],
    ['Q', 
      [x + tab.x, y].join(', '), 
      [x + tab.x + radius, y].join(', ')
    ],
  ] : [
    ['L', x + tab.x, y]
  ]

  const tabRightCorner = radius > 0 ? [
  ...
  const moveFromTab = topCornerRadius > 0 ? [
  ...
  const bottomRightCorner = radius > 0 ? [
  ...
  const bottomLeftCorner = radius > 0 ? [
    ['L', x + radius, y + height],
    ['Q', 
      [x, y + height].join(', '), 
  ...
  const closePath = radius > 0 && tabPosition !== 'left' ? [
