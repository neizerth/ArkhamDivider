...
export const withScenario = ({ scenario_encounter_sets, scenario, scenarios = [] }: IStory) =>
  Boolean(scenario) || scenario_encounter_sets.length > 0 || scenarios.length > 0;

export const isSideContent = anyPass([isSideStory, isStandalone]);
...
export const onlyWithScenarioEncounters = ({ encounter_sets, scenario_encounter_sets }: IStory) =>
  encounter_sets.filter((code) => !scenario_encounter_sets.includes(code)).length === 0;
