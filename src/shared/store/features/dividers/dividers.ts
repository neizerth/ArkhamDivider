...
export const removeDivider: ActionCreator<AppThunk> = (id: string) => (dispatch, getState) => {
  const state = getState();
...
export const copyDivider: ActionCreator<AppThunk> = (id: string) => (dispatch, getState) => {
  const data = selectDividers(getState());
...
export const addLoadIndex: ActionCreator<AppThunk> = (id: string) => (dispatch, getState) => {
  const state = getState();
  const loadQueue = selectLoadQueue(state);
...
export const { setList: setDividers, setStory, setLoadQueue, removeAllDividers } = dividers.actions;

export const { selectList: selectDividers, selectStory, selectLoadQueue } = dividers.selectors;
