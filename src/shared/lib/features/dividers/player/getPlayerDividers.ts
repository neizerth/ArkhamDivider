...
import { CardType, ICardType, IFaction, IXPCost, XPCost } from '@/shared/types/game';
...
export const getAllyType = ({ includeAllies }: { includeAllies: boolean }): ICardType[] => {
  if (!includeAllies) {
    return [];
  }
...
export const getBasicWeaknessDividers = ({ includeBasicWeakness }: AddPlayerDividersOptions) => {
