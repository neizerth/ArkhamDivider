...
export const getStoriesIconGroups = ({ stories, icons, encounterSets }: IGetIconGroupsOptions) => {
  ...
  const campaignGroups = stories.filter((story) => isMainCampaign(story) || isSideCampaign(story));
...
const getStoryIconGroup =
  ...
  ({ encounter_sets, name, icon, code, pack_codes = [], pack_code, campaigns = [] }: IStory) => {
    ...
    const icons = [icon, ...campaignIcons, ...encounter_sets.map(toIcon)].filter(isNotNil);
