...
export const isCampaign = (story: IStory) =>
	isMainCampaign(story) || isSideCampaign(story);
...
export const withReturnTo = (code: string) =>
	safePropEq(code, "return_to_code");

export const onlyWithScenarioEncounters = ({
	encounter_sets,
	scenario_encounter_sets,
}: IStory) =>
	encounter_sets.filter((code) => !scenario_encounter_sets.includes(code))
		.length === 0;
