...
export const propIncludes =  <K extends keyof U, U>(data: U[K][], name: K) => 
  (obj: U) => includes(
    prop(name, obj),
    data
  )

export const propNotIncludes =  <K extends keyof U, U>(data: U[K][], name: K) => 
  (obj: U) => includes(
    prop(name, obj),
    data
  )
...
export const safePropEq = curry(
  <T, K extends PropertyKey>(val: T, name: K, obj: Record<K, T>) => {
