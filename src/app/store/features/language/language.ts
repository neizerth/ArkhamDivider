...
import { createSliceSelector, createSliceSetter } from '@/shared/lib/features/util/slice';
...
export const changeLanguage: ActionCreator<AppThunk> = (language: string) => async (dispatch, getState) => {
...
export const addTranslatedStories: ActionCreator<AppThunk> = (language: string, translated: string[]) => async (dispatch, getState) => {
  const data = selectTranslatedStories(getState());
  
  dispatch(setTranslatedStories({
    ...data,
    [language]: translated
  }))
}
