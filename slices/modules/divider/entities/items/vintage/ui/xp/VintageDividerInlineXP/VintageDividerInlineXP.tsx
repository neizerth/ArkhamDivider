import { Box, type BoxProps } from "@mui/material";
import { range } from "ramda";
import { useId } from "react";
import { xpRangeStatuses } from "@/modules/divider/shared/config";
import { getXPRangeStatus } from "@/modules/divider/shared/lib";
import type { XPCost } from "@/modules/divider/shared/model";
import { vintageXPColors } from "../../../config/colors";

type VintageDividerInlineXPProps = BoxProps & {
	xpCost: XPCost;
};

// Scales with viewBox; parent controls final size via font-size (1em)
const dotRadius = 1.6;

const dotGap = 5.8;
const dotCenters = range(0, 5).map((i) => ({
	x: dotGap * i - 1.5,
	y: 2.0,
}));

const viewBox = { w: 23, h: 4.0 };
const aspectRatio = viewBox.w / viewBox.h;

export function VintageDividerInlineXP({
	xpCost,
	...props
}: VintageDividerInlineXPProps) {
	const baseId = useId().replace(/:/g, "");

	return (
		<Box
			{...props}
			sx={{
				display: "inline-flex",
				verticalAlign: "middle",
				...props.sx,
				height: "1em",
				width: `${aspectRatio}em`,
			}}
		>
			<svg
				viewBox={`0 0 ${viewBox.w} ${viewBox.h}`}
				width="100%"
				height="100%"
				style={{ overflow: "visible", display: "block" }}
				aria-hidden
			>
				<title>XP level</title>
				<defs>
					{xpRangeStatuses.map((status) => (
						<radialGradient
							key={status}
							id={`${baseId}-${status}`}
							cx="50%"
							cy="50%"
							r="50%"
						>
							<stop offset="0%" stopColor={vintageXPColors[status]} />
							<stop offset="80%" stopColor={vintageXPColors[status]} />
							<stop
								offset="100%"
								stopColor={vintageXPColors[status]}
								stopOpacity={0}
							/>
						</radialGradient>
					))}
				</defs>
				{dotCenters.map((p, i) => (
					<circle
						key={`${p.x}-${p.y}`}
						cx={p.x}
						cy={p.y}
						r={dotRadius}
						fill={`url(#${baseId}-${getXPRangeStatus(i, xpCost)})`}
					/>
				))}
			</svg>
		</Box>
	);
}
